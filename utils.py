...
def input_download(p_Day, p_Filename):
    ...
    with open(p_Filename, "w") as f:
        l_Request = requests.get(
                'https://adventofcode.com/2015/day/%s/input' % p_Day,
                headers={
    ...
    def draw(self, opt={}):
        for y in range(self.height):
            for x in range(self.width):
                print(opt.get((x,y), self[(x,y)]), end='')
            print()
        print()

    def all_paths_between(self, point_A, point_B, heuristic_func=cityblock_distance, moves_func=neighbors4):
        frontier  = [(heuristic_func(point_A, point_B), point_A)]
        previous  = {point_A: None}
        while frontier:
            (f, s) = heappop(frontier)
            if heuristic_func(s, point_B) == 0:
                yield Path(previous, s)
                continue
            for s2 in moves_func(s):
                heappush(frontier, (heuristic_func(s2, point_B), s2))
                previous[s2] = s

    def get_coords_for_value(self, val):
        return [
            k
...
def matrix_rotate(iter: list[list]):
    """Rotate a square string matrix"""
    return [
        [
            iter[j][i]
            for j in range(len(iter))
        ]
        for i in range(len(iter[0])-1, -1, -1)
    ]

def matrix_diagonal(m, x, y):
    """Return a list of items along diagonal top left to bottom right including the point (x,y)"""
    row = max((y - x, 0))
    col = max((x - y, 0))
    return [
        m[row + i][col + i]
...
def all_matrix_diagonals(m, rotate=False):
    """
    All the diagonals of a matrix
    If rotate is True, also include the diagonals both directions
    """
